...
public class ApiExceptionHandler {
    ...
    public ResponseEntity<MensagemErroPadrao> dadosDeEntradaInvalidosException(MethodArgumentNotValidException ex, HttpServletRequest request) {
        return ResponseEntity.status(HttpStatus.UNPROCESSABLE_ENTITY).body(new MensagemErroPadrao(System.currentTimeMillis(), 422,
                "Erro ao cadastrar funcionário", "Dados de entrada inválidos", request.getRequestURI()));
    }
